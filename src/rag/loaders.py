...
def _load_pdf_docling_batched(path: Path) -> list[Document]:
    ...
    with fitz.open(str(path)) as pdf:
        total_pages = len(pdf)
    print(
        f"[loaders] Docling por lotes: {total_pages} páginas, batch={batch_size}",
        flush=True,
    )
    ...
    for start in range(1, total_pages + 1, batch_size):
        ...
        q = ctx.Queue()
        p = ctx.Process(
            target=_docling_worker, args=(q, str(path), page_range), daemon=True
        )
        p.start()
        try:
...
def load_any(path: str | Path) -> list[Document]:
    """Dispatcher por extensión. Lanza ValueError si no hay loader registrado."""
    p = Path(path)
