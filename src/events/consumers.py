...
class RiskAlertConsumer(BaseEventConsumer):
    ...
    def __init__(
        self, *, graph: ForensicGraph, ledger: EventLedger, max_attempts: int = 5
    ) -> None:
    ...
    async def process(self, delivery: EventDelivery) -> dict[str, Any]:
        try:
            if delivery.event.type != RISK_ALERT_TYPE:
                raise ValueError(f"unexpected event type: {delivery.event.type}")
        ...
        investigation_id, request_id = (
            ...
            f"{delivery.event.id}:request:v1",
        )
        try:
            ...
            await self._retry_or_dead_letter(delivery, f"case intake failed: {exc}")
...
class EvidenceRequestedConsumer(BaseEventConsumer):
    ...
    def __init__(
        ...
        *,
        ...
        ledger: EventLedger,
        max_attempts: int = 5,
        source: str = "service://forensic-evidence-worker",
    ) -> None:
        super().__init__(ledger=ledger, max_attempts=max_attempts)
        self._collector, self._publisher, self._source = collector, publisher, source

    async def process(self, delivery: EventDelivery) -> dict[str, Any]:
        try:
            if delivery.event.type != EVIDENCE_COLLECTION_REQUESTED_TYPE:
                raise ValueError(f"unexpected event type: {delivery.event.type}")
            request = EvidenceCollectionRequested.model_validate(delivery.event.data)
        except (ValidationError, ValueError) as exc:
            await delivery.dead_letter(f"invalid evidence request: {exc}")
            return {"status": "dead_lettered"}
        if not await self._claim(delivery):
            await delivery.ack()
            return {"status": "duplicate"}
        try:
            ...
            output = make_event(
                event_type=EVIDENCE_READY_TYPE,
                source=self._source,
            ...
            await self._retry_or_dead_letter(
                delivery, f"evidence collection failed: {exc}"
            )
            return {"status": "retrying"}
        await self._ledger.complete(delivery.event.id)
        await delivery.ack()
...
class EvidenceReadyConsumer(BaseEventConsumer):
    ...
    async def process(self, delivery: EventDelivery) -> dict[str, Any]:
        try:
            if delivery.event.type != EVIDENCE_READY_TYPE:
                raise ValueError(f"unexpected event type: {delivery.event.type}")
            ready = EvidenceReady.model_validate(delivery.event.data)
        except (ValidationError, ValueError) as exc:
            await delivery.dead_letter(f"invalid evidence-ready event: {exc}")
            return {"status": "dead_lettered"}
        if not await self._claim(delivery):
            await delivery.ack()
            return {"status": "duplicate"}
        try:
            ...
            await self._ledger.release(delivery.event.id)
            await self._retry_or_dead_letter(delivery, f"case reasoning failed: {exc}")
            return {"status": "retrying"}
        await self._ledger.complete(delivery.event.id)
        await delivery.ack()
        return result
