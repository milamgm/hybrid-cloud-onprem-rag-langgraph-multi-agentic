...
def _dependencies(bus):
    return ForensicGraphDependencies(
        evidence_publisher=bus,
        review_publisher=bus,
        approval_granted_publisher=bus,
        execution_publisher=bus,
        reasoner=MockForensicReasoner(),
    )


def _collector() -> CaseEvidenceCollector:
    return CaseEvidenceCollector(
        core_banking=MockCoreBankingReadGateway(), policy_rag=MockPolicyRAGGateway()
    )
...
def test_alert_is_enriched_and_reported_before_human_interrupt() -> None:
    async def scenario() -> None:
        ...
        evidence = EvidenceRequestedConsumer(
            collector=_collector(), publisher=bus, ledger=ledger
        )
        ready = EvidenceReadyConsumer(graph=graph, ledger=ledger)
        async with (
            bus.subscribe(RISK_ALERT_TYPE, "intake") as alert_sub,
            bus.subscribe(
                EVIDENCE_COLLECTION_REQUESTED_TYPE, "evidence"
            ) as request_sub,
            bus.subscribe(EVIDENCE_READY_TYPE, "reasoning") as ready_sub,
            bus.subscribe(APPROVAL_REQUESTED_TYPE, "analyst-work-queue") as review_sub,
        ):
...
def test_human_reviewer_can_resume_a_customer_case_without_becoming_state_scope() -> (
    None
):
    async def scenario() -> None:
        bus, ledger = InMemoryEventBus(), InMemoryEventLedger()
        ...
        async with (
            ...
            bus.subscribe(EVIDENCE_READY_TYPE, "reasoning") as ready_sub,
            bus.subscribe(APPROVAL_REQUESTED_TYPE, "analyst") as review_sub,
            bus.subscribe(HUMAN_APPROVAL_GRANTED_TYPE, "audit") as granted_sub,
            bus.subscribe(EXECUTION_ORDER_REQUESTED_TYPE, "executor") as execution_sub,
        ):
            await MockXGBoostModel(bus).score_and_publish(_sample("tx-hitl"))
            await alerts.process(await alert_sub.__anext__())
            await evidence.process(await request_sub.__anext__())
            await ready.process(await ready_sub.__anext__())
            review = await review_sub.__anext__()
            ...
            assert (
                status.interrupts[0]["report_id"] == review.event.data.report.report_id
            )
            resumed = await service.resume(
                thread_id=case_id,
                tenant_id="tenant-a",
                approver_ref="analyst-007",
                approver_roles=("risk.investigation.approve",),
                approval_request_id=status.approval_request_id,
                decision="approved",
                reason="Evidence and cited procedure reviewed.",
            )
            assert resumed.status == "execution_requested"
            assert (
                await granted_sub.__anext__()
            ).event.data.approver_ref == "analyst-007"
    ...
    async def scenario() -> None:
        ...
        async with bus.subscribe(RISK_ALERT_TYPE, "intake") as subscription:
            event = await MockXGBoostModel(bus).score_and_publish(
                _sample("tx-duplicate")
            )
            delivery = await subscription.__anext__()
            first, second = (
                await consumer.process(delivery),
                await consumer.process(delivery),
            )
        assert first["stage"] == "evidence_requested" and second == {
            "status": "duplicate"
        }
        assert [item.type for item in bus.published] == [
            event.type,
            EVIDENCE_COLLECTION_REQUESTED_TYPE,
        ]
...
def test_topology_defaults_are_mode_specific(monkeypatch) -> None:
    ...
    assert EventTopologySettings.from_env().stream_backend == "azure_event_hubs"
    monkeypatch.setenv("INFRASTRUCTURE_MODE", "onprem")
    assert EventTopologySettings.from_env().transactional_backend == "rabbitmq"
    ...
    async def scenario() -> None:
        ...
        class NeverCalledGraph:
            async def ainvoke(self, input, *, config):
                raise AssertionError("invalid payload must not reach graph")
